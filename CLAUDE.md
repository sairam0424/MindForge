...
To skip a test, make its **first line** `// @skip: reason`; to raise its timeout, `// @timeout: <ms>` (default `60000`). There is no Jest/Mocha — `tests/run-all.js` walks `tests/` **recursively** and runs every file whose name ends in `.test.js`, sequentially, each in its own `node` process with the repo root as `cwd`, propagating exit codes. **111 files today: 109 pass, 2 env-dependent skips (`browser.test.js`, `sre-integration.test.js`), 0 failures.** A run that discovers zero files exits **1**, not 0 — see `tests/run-all-floor.test.js`. Directories named `tmp-*`, `node_modules` or `.*` are pruned — that prune applies to **directories only**, so a *file* named `tmp-foo.test.js` still runs; use `// @skip:` to exclude a test, never a filename prefix. `--filter=` matches the whole `tests/`-relative path, so it can select a subdirectory; a `--filter` that selects **nothing** now exits 1 — pass `--allow-empty` when an empty selection is intended. The module exports `discoverTests`/`getSkipReason`/`getTimeoutMs` and `main()` is behind a `require.main` guard, so requiring it does not start a run.

**Never bump versions by hand — run `node scripts/sync-version.js`.** `package.json` is canonical and **12 other files** are derived from it: `sdk/package.json`, `mcp-server/package.json`, `.mindforge/config.json`, all three `package-lock.json`s (top-level *and* `packages[""]`), `sdk/src/index.ts`, `sdk/README.md`, `MINDFORGE.md` (`[VERSION]` + H1 title), `.claude-plugin/marketplace.json` (the `mindforge` entry **only**), `Dockerfile` (the `ARG` default — what `docker build .` uses with no flag) and `Formula/mindforge.rb` (url + sha256 + `assert_match`; pass `--sha256 <hex>` or `--fetch-sha`, since the digest cannot be derived offline). `npm run version:check` is the offline `--check` mode, wired into the `mindforge-quality` CI job; `tests/version-consistency.test.js` asserts every channel.

This replaced a claim that bumps "touch 5 files". Trusting it is why 11.9.2 shipped with the Homebrew formula and `Dockerfile` still pinned to **11.5.1** and the marketplace entry to **11.4.0** — four releases behind — while all 12 npm-relevant locations were correctly in sync. Two deliberate exclusions, both asserted rather than merely omitted: `MINDFORGE.md`'s `[REQUIRED_CORE_VERSION]` is a *minimum floor* with no code readers, so it may lag but must never exceed canonical; and the **10 subagent-category entries** in `marketplace.json` version on their own `1.x` line and must never be swept (a regex over that file would silently republish all ten).
