export const meta = {
  name: "release-prep",
  description:
    "Automated release pipeline: tests → changelog → version bump → PR → announcement draft",
  whenToUse:
    "When preparing a production release and need all release artifacts generated",
  phases: [
    {
      title: "Check",
      detail: "Verify tests pass, no uncommitted changes, CI status",
    },
    {
      title: "Changelog",
...
log(
  `Readiness: ${readiness.ready ? "GO" : "BLOCKED"} — ${readiness.blockers.length} blockers, ${readiness.warnings.length} warnings`,
);
...
log(
  `Changelog: ${changelog.breaking.length} breaking, ${changelog.features.length} features, ${changelog.fixes.length} fixes`,
);
