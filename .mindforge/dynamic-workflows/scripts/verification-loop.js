export const meta = {
  ...
  description:
    "Implementer vs. scorer loop: one agent revises an artifact each round, a separate agent scores it against a fixed metric, and the implementer keeps re-attempting until the score is maxed or a round cap is hit",
  whenToUse:
    'When a task has a single well-defined quality metric worth pushing as high as possible, rather than a single fixed-count review pass — e.g. tightening a prompt, hardening a config, polishing a spec, or any "keep improving this until the score stops moving" task',
  phases: [
    {
      title: "Scope",
    ...
    },
    {
      title: "Round",
      detail:
        "implementer revises, scorer grades — repeats until target hit, score plateaus, or cap reached",
    },
    {
      title: "Report",
...
const SCORE_SCHEMA = {
  ...
  properties: {
    score: {
      type: "number",
      description:
        '0-100. Same metric every round — do not silently redefine what "good" means between rounds.',
    },
    breakdown: {
      type: "array",
      items: {
    ...
    topFixes: {
      ...
      items: { type: "string" },
      description:
        "the highest-leverage concrete changes that would raise the score next round, ordered by impact — not a full audit, just what to fix next",
    },
  },
...
const REVISE_SCHEMA = {
  ...
  properties: {
    artifact: {
      type: "string",
      description:
        "the full revised artifact — not a diff, not a description of changes, the complete thing so the next scorer round has the real content",
    },
...
log(
  `Target: ${targetScore}/100, max ${maxRounds} rounds, stop after ${plateauRounds} flat rounds`,
);
...
for (let round = 1; round <= maxRounds; round++) {
  if (budget.total && budget.remaining() < 20_000) {
    log(
      `Stopping at round ${round - 1}: token budget nearly exhausted (${Math.round(budget.remaining() / 1000)}k left)`,
    );
    break;
  }
  ...
  const revision = await agent(revisePrompt, {
  ...
  const scored = await agent(scorePrompt, {
    label: `score:r${round}`,
    phase: "Round",
    schema: SCORE_SCHEMA,
  });
  ...
  if (scored.score <= prevScore) {
    ...
    if (roundsFlat >= plateauRounds) {
      log(
        `Score has not improved for ${plateauRounds} consecutive rounds — stopping (diminishing returns, not a bug).`,
      );
      break;
    }
  } else {
...
log(
  `Final score: ${finalScore.score}/100 across ${scoreHistory.length} round(s)`,
);

return {
  task,
  metric:
    metric || "(scorer-inferred, see round 1 breakdown for the rubric it held)",
  finalArtifact: currentArtifact,
  ...
  stoppedBecause:
    finalScore.score >= targetScore
      ? "target-reached"
      : roundsFlat >= plateauRounds
        ? "plateaued"
        : "max-rounds",
};
