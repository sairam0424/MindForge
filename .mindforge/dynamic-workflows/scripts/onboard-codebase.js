export const meta = {
  ...
  description:
    "Map structure → domain analysis → architecture → generate guided tour and onboarding docs",
  whenToUse:
    "When joining a new codebase or onboarding a team member to an existing repo",
  phases: [
    {
      title: "Map",
  ...
  properties: {
    domains: {
      ...
      items: {
        ...
        required: ["name", "description", "keyFiles"],
      },
    },
...
log(
  `${map.primaryLanguage} codebase, ${map.frameworks.join("+")} — ${map.keyDirectories.length} key directories`,
);
...
log(
  `${domain.domains.length} domains: ${domain.domains.map((d) => d.name).join(", ")}`,
);
