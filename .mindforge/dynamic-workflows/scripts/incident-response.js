export const meta = {
  ...
  description:
    "Parallel investigation across logs, metrics, traces, and code → mitigation → RCA → postmortem",
  whenToUse:
    "When responding to a production incident or unexpected system failure",
  phases: [
    {
      title: "Alert",
...
const incident =
  args ||
  "No incident description provided — pass your incident description, symptoms, and affected systems as args.";
...
log(
  `[${characterization.severity}] ${characterization.incidentTitle} — ${characterization.symptoms.length} symptoms identified`,
);
...
phase("Mitigate");
const investigationSummary = investigations
  .filter(Boolean)
  .map(
    (i) =>
      `[${i.angle}] ${i.suspectedCause} (${i.confidence} confidence): ${i.findings.slice(0, 2).join(", ")}`,
  )
  .join("\n");
