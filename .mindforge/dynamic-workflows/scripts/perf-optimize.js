export const meta = {
  name: "perf-optimize",
  description:
    "Profile → parallel bottleneck hunt across DB/network/CPU/memory → prioritized fix plan",
  whenToUse:
    "When an application is slow and you need to find and fix performance bottlenecks",
  phases: [
    {
      title: "Profile",
  ...
  properties: {
    slowestPaths: {
      ...
      items: {
        ...
        properties: {
          path: { type: "string" },
          currentLatency: { type: "string" },
          target: { type: "string" },
        },
        required: ["path", "currentLatency"],
      },
    },
...
log(
  `${profile.slowestPaths.length} slow paths identified, hypothesis: ${profile.bottleneckHypothesis.slice(0, 80)}`,
);
...
phase("Plan");
const allIssues = bottlenecks
  .filter(Boolean)
  .flatMap((b) => b.issues.map((i) => ({ ...i, dimension: b.dimension })));
log(
  `${allIssues.length} bottlenecks found across ${bottlenecks.filter(Boolean).length} dimensions`,
);
const issueList = allIssues
  .slice(0, 20)
  .map(
    (i) =>
      `[${i.dimension}][${i.impact}/${i.effort}] ${i.description} → ${i.fix}`,
  )
  .join("\n");
...
if (!fixPlan) {
  return {
    ...
    error: "fixPlan-agent-null",
  };
}

phase("Benchmark");
const topFixes = fixPlan.prioritized
  .slice(0, 5)
  .map((f) => `${f.rank}. ${f.fix} (expected: ${f.expectedImprovement})`)
  .join("\n");
