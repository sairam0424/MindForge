export const meta = {
  name: "refactor-plan",
  description:
    "Technical debt scan → risk-sorted sequence → safe refactor implementation plan",
  whenToUse:
    "When you need to plan a refactoring safely without breaking existing behavior",
  phases: [
    {
      title: "Scan",
...
const SCAN_AREAS = [
  {
    label: "structural",
    prompt: `Scan for structural technical debt in: "${target}". Look for: God objects/files (>400 lines), circular dependencies, tight coupling, missing abstraction layers, inconsistent patterns across similar features.`,
  },
  {
    label: "complexity",
    prompt: `Scan for complexity debt in: "${target}". Look for: functions >50 lines, nesting >4 levels, cyclomatic complexity, boolean parameter flags, long parameter lists, feature envy, shotgun surgery patterns.`,
  },
  {
    label: "maintenance",
    prompt: `Scan for maintenance debt in: "${target}". Look for: dead code, commented-out code, outdated comments, TODO/FIXME/HACK markers, deprecated API usage, inconsistent naming, magic numbers/strings.`,
  },
];
...
log(
  `Found ${allDebts.length} debt items across ${scans.filter(Boolean).length} areas`,
);
const debtList = allDebts
  .slice(0, 20)
  .map(
    (d, i) =>
      `${i + 1}. [blast:${d.blastRadius}][coverage:${d.testCoverage}] ${d.description}`,
  )
  .join("\n");
...
if (!prioritized) {
  return {
    ...
    error: "prioritized-agent-null",
  };
}

phase("Sequence");
const topItems = prioritized.prioritized
  .slice(0, 10)
  .map((p) => `${p.rank}. [${p.risk}] ${p.description}: ${p.rationale}`)
  .join("\n");
