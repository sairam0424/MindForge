...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { mindforge_state_version: '1.0' };
...
/**
 * Write a WAITING.json signal file when MindForge hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .mindforge/WAITING.json if .mindforge exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const mindforgeDir = fs.existsSync(path.join(cwd, '.mindforge')) ? path.join(cwd, '.mindforge') : planningDir(cwd);
  const waitingPath = path.join(mindforgeDir, 'WAITING.json');
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.mindforge', 'WAITING.json'),
