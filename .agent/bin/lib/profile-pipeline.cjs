...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
  ...
  reapStaleTempFiles('mindforge-pipeline-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mindforge-pipeline-'));
...
async function cmdProfileSample(overridePath, options, raw) {
  ...
  reapStaleTempFiles('mindforge-profile-', { dirsOnly: true });
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mindforge-profile-'));
