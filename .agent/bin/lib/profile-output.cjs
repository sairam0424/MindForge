...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /mindforge-new-project to set up.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- MindForge:${sectionName}-start`;
  const endMarker = `<!-- MindForge:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- MindForge:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- MindForge:${sectionName}-end -->`,
  ].join('\n');
}
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'mindforge', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/mindforge-profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'mindforge', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/mindforge-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by MindForge from ${dataSource}. Run \`/mindforge-profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- MindForge:profile-start -->';
    const endMarker = '<!-- MindForge:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## MindForge Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- MindForge:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- MindForge:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- MindForge:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- MindForge:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /mindforge-profile-user to unlock Developer Profile.';
