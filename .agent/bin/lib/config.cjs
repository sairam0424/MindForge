 ...
 *   2. User-level defaults from ~/.mindforge/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /mindforge-new-project
 *
...
function buildNewProjectConfig(userChoices) {
  ...
  // Load user-level defaults from ~/.mindforge/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.mindforge', 'defaults.json');
  let userDefaults = {};
  try {
 ...
 * configured during /mindforge-new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.mindforge/defaults.json.
 *
