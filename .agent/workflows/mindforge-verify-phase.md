...
<required_reading>
@.agent/references/verification-patterns.md
...
```bash
node ".agent/bin/mindforge-tools.cjs" roadmap get-phase "${phase_number}"
...
Use mindforge-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ".agent/bin/mindforge-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ".agent/bin/mindforge-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use mindforge-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ".agent/bin/mindforge-tools.cjs" verify artifacts "$plan")
...
Use mindforge-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ".agent/bin/mindforge-tools.cjs" verify key-links "$plan")
...
See .agent/mindforge/templates/verification-report.md for complete template.
</step>
