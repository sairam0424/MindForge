...
2. **Run audit first** — `/mindforge-audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ".agent/bin/mindforge-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `mindforge-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to mindforge-tools:**

```bash
ARCHIVE=$(node ".agent/bin/mindforge-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/mindforge-cleanup` later to archive retroactively.
...
**If doesn't exist:** Create from template at `.agent/mindforge/templates/retrospective.md`.
...
**Commit:**
```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node ".agent/bin/mindforge-tools.cjs" init execute-phase "1")
...
```bash
node ".agent/bin/mindforge-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/mindforge-new-milestone)
