<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/mindforge-resume-work`; the markdown provides human-readable context.
</purpose>
...
```bash
timestamp=$(node ".agent/bin/mindforge-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node ".agent/bin/mindforge-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md .planning/HANDOFF.json
```
</step>
...
To resume: /mindforge-resume-work
