<purpose>
Check for MindForge updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether MindForge is installed locally or globally by checking both locations and validating install integrity.
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/mindforge-update` targets the runtime that invoked it.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/mindforge/VERSION" ] || [ -f "./$dir/mindforge/workflows/mindforge-update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/mindforge/VERSION"
    LOCAL_MARKER_FILE="./$dir/mindforge/workflows/mindforge-update.md"
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/mindforge/VERSION" ] || [ -f "$HOME/$dir/mindforge/workflows/mindforge-update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/mindforge/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/mindforge/workflows/mindforge-update.md"
...
```bash
npm view mindforge-cc version 2>/dev/null
```
...
To update manually: `npx mindforge-cc --global`
```
...
⚠️  **Note:** The installer performs a clean install of MindForge folders:
- `commands/mindforge/` will be wiped and replaced
- `mindforge/` will be wiped and replaced
- `agents/mindforge-*` files will be replaced
...
- Custom commands not in `commands/mindforge/` ✓
- Custom agents not prefixed with `mindforge-` ✓
- Custom hooks ✓
- Your GEMINI.md files ✓

If you've modified any MindForge files directly, they'll be automatically backed up to `mindforge-local-patches/` and can be reapplied with `/mindforge-reapply-patches` after the update.
```
...
**If LOCAL install:**
```bash
npx -y mindforge-cc@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y mindforge-cc@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y mindforge-cc@latest --claude --global
```
...
for dir in .claude .config/opencode .opencode .gemini .codex; do
  rm -f "./$dir/cache/mindforge-update-check.json"
  rm -f "$HOME/$dir/cache/mindforge-update-check.json"
done
```

The SessionStart hook (`mindforge-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  MindForge Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/sairam0424/MindForge/blob/main/CHANGELOG.md) ([full archive](https://github.com/sairam0424/MindForge/blob/main/changelogs/))
```
</step>
...
Check for mindforge-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /mindforge-reapply-patches to merge your modifications into the new version.
```
