...
- **Claude Code:** Uses `Task(subagent_type="mindforge-executor", ...)` — blocks until complete, returns result
...
These are the valid MindForge subagent types registered in .agent/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- mindforge-executor — Executes plan tasks, commits, creates SUMMARY.md
- mindforge-verifier — Verifies phase completion, checks quality gates
...
- mindforge-debugger — Diagnoses and fixes issues
- mindforge-codebase-mapper — Maps project structure and dependencies
- mindforge-integration-checker — Checks cross-phase integration
- mindforge-nyquist-auditor — Validates verification coverage
- mindforge-ui-researcher — Researches UI/UX approaches
- mindforge-ui-checker — Reviews UI implementation quality
- mindforge-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
INIT=$(node ".agent/bin/mindforge-tools.cjs" init execute-phase "${PHASE_ARG}")
...
Check if the current runtime is Copilot by testing for the `@mindforge-executor` agent pattern
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `.agent/mindforge/workflows/mindforge-execute-plan.md` **inline**
...
- Maintains MindForge's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning MindForge
...
```bash
node ".agent/bin/mindforge-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node ".agent/bin/mindforge-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   ```
   Task(
     subagent_type="mindforge-executor",
     ...
     prompt="
       ...
       For mindforge-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @.agent/workflows/mindforge-execute-plan.md
       @.agent/templates/summary.md
       @.agent/references/checkpoints.md
       @.agent/references/tdd.md
       </execution_context>
...
5. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a MindForge or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node ".agent/bin/mindforge-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
POST_PLAN_INDEX=$(node ".agent/bin/mindforge-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
/mindforge-execute-phase {phase} ${MindForge_WS}                # Continue remaining waves
/mindforge-execute-phase {phase} --wave {next} ${MindForge_WS}  # Run the next wave explicitly
```
...
```bash
PARENT_INFO=$(node ".agent/bin/mindforge-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
Create VERIFICATION.md.",
  subagent_type="mindforge-verifier",
...
| `gaps_found` | Present gap summary, offer `/mindforge-plan-phase {phase} --gaps ${MindForge_WS}` |
...
Commit the file:
```bash
node ".agent/bin/mindforge-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/mindforge-progress` and `/mindforge-audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/mindforge-verify-work` on it.
...
`/mindforge-plan-phase {X} --gaps ${MindForge_WS}`

<sub>`/clear` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/mindforge-verify-work {X} ${MindForge_WS}` — manual testing first
```

Gap closure cycle: `/mindforge-plan-phase {X} --gaps ${MindForge_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/mindforge-execute-phase {X} --gaps-only ${MindForge_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(node ".agent/bin/mindforge-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/mindforge-progress` and `/mindforge-audit-uat`.
```

```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/mindforge-plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `.agent/mindforge/workflows/mindforge-transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/mindforge-transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/mindforge-progress ${MindForge_WS} — see updated roadmap
/mindforge-discuss-phase {next} ${MindForge_WS} — discuss next phase before planning
/mindforge-plan-phase {next} ${MindForge_WS} — plan next phase
/mindforge-execute-phase {next} ${MindForge_WS} — execute next phase
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not MindForge. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/mindforge-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
