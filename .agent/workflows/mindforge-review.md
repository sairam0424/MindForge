...
Then run /mindforge-review again.
```
Exit.
...
Write to a temp file: `/tmp/mindforge-review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/mindforge-review-prompt-{phase}.md)" 2>/dev/null > /tmp/mindforge-review-gemini-{phase}.md
```

**the agent (separate session):**
```bash
claude -p "$(cat /tmp/mindforge-review-prompt-{phase}.md)" --no-input 2>/dev/null > /tmp/mindforge-review-claude-{phase}.md
```

**Codex:**
```bash
codex -p "$(cat /tmp/mindforge-review-prompt-{phase}.md)" 2>/dev/null > /tmp/mindforge-review-codex-{phase}.md
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 MindForge ► CROSS-AI REVIEW — Phase {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Commit:
```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
To incorporate feedback into planning:
  /mindforge-plan-phase {N} --reviews
```
...
- [ ] User knows how to use feedback (/mindforge-plan-phase --reviews)
</success_criteria>
