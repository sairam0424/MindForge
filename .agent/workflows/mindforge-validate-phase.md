...
```bash
AUDITOR_MODEL=$(node ".agent/bin/mindforge-tools.cjs" resolve-model mindforge-nyquist-auditor --raw)
NYQUIST_CFG=$(node ".agent/bin/mindforge-tools.cjs" config-get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /mindforge-settings."

Display banner: `MindForge > VALIDATE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /mindforge-execute-phase {N} ${MindForge_WS} first."
...
## 5. Spawn mindforge-nyquist-auditor

```
Task(
  prompt="Read .agent/agents/mindforge-nyquist-auditor.md for instructions.\n\n" +
    "<files_to_read>{PLAN, SUMMARY, impl files, VALIDATION.md}</files_to_read>" +
    "<gaps>{gap list}</gaps>" +
    "<test_infrastructure>{framework, config, commands}</test_infrastructure>" +
    "<constraints>Never modify impl files. Max 3 debug iterations. Escalate impl bugs.</constraints>",
  subagent_type="mindforge-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `.agent/mindforge/templates/VALIDATION.md`
...
node ".agent/bin/mindforge-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
MindForge > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /mindforge-audit-milestone ${MindForge_WS}
```

**Partial:**
```
MindForge > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /mindforge-validate-phase {N} ${MindForge_WS}
```
