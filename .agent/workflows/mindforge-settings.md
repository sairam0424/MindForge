<purpose>
Interactive configuration of MindForge workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.mindforge/defaults.json) for future projects.
</purpose>
...
```bash
node ".agent/bin/mindforge-tools.cjs" config-ensure-section
...
- `workflow.ui_safety_gate` — prompt to run /mindforge-ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /mindforge-ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /mindforge-ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (mindforge/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (mindforge/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /mindforge-autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  }
])
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.mindforge/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.mindforge/defaults.json`:

```bash
mkdir -p ~/.mindforge
```

Write `~/.mindforge/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /mindforge-plan-phase and /mindforge-execute-phase runs.

Quick commands:
- /mindforge-set-profile <profile> — switch model profile
- /mindforge-plan-phase --research — force research
- /mindforge-plan-phase --skip-research — skip research
- /mindforge-plan-phase --skip-verify — skip plan check
```
</step>
...
- [ ] User offered to save as global defaults (~/.mindforge/defaults.json)
