...
```bash
INIT=$(node ".agent/bin/mindforge-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /mindforge-new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /mindforge-new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /mindforge-new-project    # Initialize MindForge in the workspace
```
...
**Offer to initialize MindForge (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize MindForge"
- question: "Would you like to initialize a MindForge project in the new workspace?"
- options:
  - "Yes — run /mindforge-new-project" → tell user to `cd $TARGET_PATH` first, then run `/mindforge-new-project`
