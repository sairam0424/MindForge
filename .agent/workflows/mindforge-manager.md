...
```bash
INIT=$(node ".agent/bin/mindforge-tools.cjs" init manager)
```
...
All {phase_count} phases done. Ready for final steps:
  → /mindforge-verify-work — run acceptance testing
  → /mindforge-complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="mindforge-verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="mindforge-complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in MindForge config (default: 60 seconds, set to 0 to disable).
...
```
Skill(skill="mindforge-discuss-phase", args="{PHASE_NUM}")
```
...
Task(
  ...
  prompt="You are running the MindForge plan-phase workflow for phase {N} of the project.
...
Steps:
1. Read the plan-phase workflow: cat .agent/mindforge/workflows/mindforge-plan-phase.md
2. Run: node \".agent/bin/mindforge-tools.cjs\" init plan-phase {N}
...
5. Spawn a mindforge-planner subagent via Task() to create the plans.
6. If plan-checker is enabled, spawn a mindforge-plan-checker subagent to verify.
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the MindForge execute-phase workflow for phase {N} of the project.
...
Steps:
1. Read the execute-phase workflow: cat .agent/mindforge/workflows/mindforge-execute-phase.md
2. Run: node \".agent/bin/mindforge-tools.cjs\" init execute-phase {N}
3. Follow the workflow steps: discover plans, analyze dependencies, group into waves.
4. For each wave, spawn mindforge-executor subagents via Task() to execute plans in parallel.
5. After all waves complete, spawn a mindforge-verifier subagent if verifier is enabled.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 MindForge ► SESSION END
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 {milestone_version} — {milestone_name}
 {PROGRESS_BAR} {progress_pct}%  ({completed_count}/{phase_count} phases)

 Resume anytime: /mindforge-manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/mindforge-manager` or `/mindforge-progress` invocation.
