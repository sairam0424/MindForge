...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/mindforge-discuss-phase --text`)
- Per-project: `mindforge-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/mindforge-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
Use /mindforge-progress ${MindForge_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /mindforge-plan-phase {X} ${MindForge_WS} to replan
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node ".agent/bin/mindforge-tools.cjs" resolve-model mindforge-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @.agent/agents/mindforge-advisor-researcher.md for your role and instructions.
...
`/mindforge-plan-phase ${PHASE} ${MindForge_WS}`
...
**Also available:**
- `/mindforge-plan-phase ${PHASE} --skip-research ${MindForge_WS}` — plan without research
- `/mindforge-ui-phase ${PHASE} ${MindForge_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```
Skill(skill="mindforge-plan-phase", args="${PHASE} --auto ${MindForge_WS}")
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /mindforge-execute-phase ${PHASE} ${MindForge_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /mindforge-plan-phase ${PHASE} ${MindForge_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /mindforge-plan-phase ${PHASE} --gaps ${MindForge_WS}
  ```
