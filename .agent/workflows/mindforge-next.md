<purpose>
Detect current project state and automatically advance to the next logical MindForge workflow step.
...
```bash
# Get state snapshot
node ".agent/bin/mindforge-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```
No MindForge project detected. Run `/mindforge-new-project` to get started.
```
Exit.
</step>
...
→ Next action: `/mindforge-discuss-phase <first-phase>`
...
▶ **Next step:** `/mindforge-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/mindforge-next` is zero-friction advancement.
</step>
