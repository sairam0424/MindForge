...
There is no `/mindforge-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/mindforge-transition`.

**Valid user commands for phase progression:**
- `/mindforge-discuss-phase {N}` — discuss a phase before planning
- `/mindforge-plan-phase {N}` — plan a phase
- `/mindforge-execute-phase {N}` — execute a phase
- `/mindforge-progress` — see roadmap progress
...
These will carry forward as debt. Review: `/mindforge-audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to mindforge-tools:**

```bash
TRANSITION=$(node ".agent/bin/mindforge-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `mindforge-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ".agent/bin/mindforge-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `mindforge-tools phase complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `MindForge_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$MindForge_WORKSTREAM" ]; then
  WS_LIST=$(node ".agent/bin/mindforge-tools.cjs" workstream list --raw)
fi
```
...
Filter out the current workstream (`$MindForge_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/mindforge-plan-phase [X+1] --auto ${MindForge_WS}")
...
Exit skill and invoke SlashCommand("/mindforge-discuss-phase [X+1] --auto ${MindForge_WS}")
...
`/mindforge-discuss-phase [X+1] ${MindForge_WS}` — gather context and clarify approach
...
**Also available:**
- `/mindforge-plan-phase [X+1] ${MindForge_WS}` — skip discussion, plan directly
- `/mindforge-research-phase [X+1] ${MindForge_WS}` — investigate unknowns
...
`/mindforge-plan-phase [X+1] ${MindForge_WS}`
...
**Also available:**
- `/mindforge-discuss-phase [X+1] ${MindForge_WS}` — revisit context
- `/mindforge-research-phase [X+1] ${MindForge_WS}` — investigate unknowns
...
```bash
node ".agent/bin/mindforge-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/mindforge-workstreams complete {current_ws_name} ${MindForge_WS}`

See overall milestone progress:

`/mindforge-workstreams progress ${MindForge_WS}`
...
Do NOT suggest `/mindforge-complete-milestone` or `/mindforge-new-milestone`.
...
Exit skill and invoke SlashCommand("/mindforge-complete-milestone {version} ${MindForge_WS}")
...
`/mindforge-complete-milestone {version} ${MindForge_WS}`
