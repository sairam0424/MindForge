...
```bash
CONFIG=$(node ".agent/bin/mindforge-tools.cjs" state load)
```
...
```bash
node ".agent/bin/mindforge-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node ".agent/bin/mindforge-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
- /mindforge-complete-milestone (if last phase in milestone)
- /mindforge-progress (to see what's next)
...
- /mindforge-complete-milestone — if all phases in milestone are done
- /mindforge-progress — see overall project state
- /mindforge-execute-phase {next} — continue to next phase
</offer_next>
