...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing mindforge-tools.cjs subcommands and the mindforge-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @.agent/references/ui-brand.md (display patterns)
- @.agent/agents/mindforge-user-profiler.md (profiler agent definition)
- @.agent/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
cp ".agent/mindforge/USER-PROFILE.md" ".agent/mindforge/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 MindForge > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ Stores profile at .agent/mindforge/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /mindforge-profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node .agent/bin/mindforge-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .agent/bin/mindforge-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn mindforge-user-profiler agent using Task tool:**

Use the Task tool to spawn the `mindforge-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `.agent/mindforge/references/user-profiling.md`
...
Reference: @.agent/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .agent/bin/mindforge-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/mindforge-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node .agent/bin/mindforge-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/mindforge-profile-analysis-XXXXXX.json)
```
...
```bash
node .agent/bin/mindforge-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .agent/mindforge/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/mindforge-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .agent/mindforge/USER-PROFILE.md" and jump to step 10.
...
**For /mindforge-dev-preferences (if selected):**

```bash
node .agent/bin/mindforge-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /mindforge-dev-preferences at .agent/commands/mindforge/dev-preferences.md"

**For GEMINI.md profile section (if selected):**

```bash
node .agent/bin/mindforge-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .agent/bin/mindforge-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to .agent/GEMINI.md"

**Error handling:** If any mindforge-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH=".agent/mindforge/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 MindForge > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    .agent/mindforge/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /mindforge-dev-preferences   .agent/commands/mindforge/dev-preferences.md
...
- [ ] Artifacts generated sequentially via mindforge-tools.cjs subcommands
