...
Seeds auto-surface during /mindforge-new-milestone when trigger conditions match the
...
This seed should be presented during `/mindforge-new-milestone` when the milestone
...
```bash
node ".agent/bin/mindforge-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /mindforge-new-milestone
