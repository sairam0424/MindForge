...
async function main() {
  ...
  test('SDK VERSION matches 11.5.0', () => {
    const src = fs.readFileSync(path.join(__dirname, '..', 'sdk', 'src', 'index.ts'), 'utf8');
    assert.ok(src.includes('VERSION = \'11.5.0\''));
  });
  ...
  test('SDK package.json version is 11.5.0', () => {
    const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'sdk', 'package.json'), 'utf8'));
    assert.strictEqual(pkg.version, '11.5.0');
  });

  // ── Package Metadata ───────────────────────────────────────────────────────
  console.log('\nPackage Metadata:');

  test('package.json version is 11.5.0', () => {
    const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'package.json'), 'utf8'));
    assert.strictEqual(pkg.version, '11.5.0');
  });

  test('MINDFORGE.md version is 11.5.0', () => {
    const md = fs.readFileSync(path.join(__dirname, '..', 'MINDFORGE.md'), 'utf8');
    assert.ok(md.includes('11.5.0'));
  });
