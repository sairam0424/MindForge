...
test('imported-agents.jsonl exists and has exactly 164 non-empty lines', () => {
  assert.ok(fs.existsSync(INDEX_PATH), `index missing: ${INDEX_PATH}`);
  const entries = loadIndexEntries();
  assert.strictEqual(entries.length, 164,
    `expected 164 indexed subagents, got ${entries.length}`);
});
...
// on the claude runtime and an installSubagents() that flattens the 164 files
...
test('installer flattens 164 subagents into a native agents dir (READMEs excluded)', () => {
  ...
  // basename, so a flat copy yields exactly 164 files with no collisions.
  ...
  walk(categoriesDir);
  assert.strictEqual(agentFiles.length, 164,
    `expected 164 installable agent files, got ${agentFiles.length}`);
  const unique = new Set(agentFiles);
  assert.strictEqual(unique.size, 164,
