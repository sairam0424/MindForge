...
if (!SOURCE) {
  console.log(
    'Usage: node bin/skills-builder/learn-cli.js <source-url-or-path> [skill-name]',
  );
  process.exit(1);
}
...
async function main() {
  try {
    ...
    console.log(
      `⭐ Quality Score: ${scoreResult.quality_score}/100 (${scoreResult.threshold_status})`,
    );
    ...
    if (scoreResult.can_register) {
      ...
      scoreResult.improvement_suggestions.forEach((s) =>
        console.log(`  - ${s}`),
      );
    }
  } catch (err) {
