...
This reference document defines detection heuristics for behavioral profiling across 8 dimensions. MindForge-user-profiler agent applies these rules when analyzing extracted session messages. Do not invent dimensions or scoring rules beyond what is defined here.

## How to Use This Document

1. MindForge-user-profiler agent reads this document before analyzing any messages
2. For each dimension, MindForge scans messages for the signal patterns defined below
3. MindForge applies the detection heuristics to classify the developer's pattern
...
**What we're measuring:** How the developer phrases requests, instructions, and feedback -- the structural pattern of their messages to MindForge.
...
**What we're measuring:** How quickly the developer makes choices when MindForge presents options, alternatives, or trade-offs.
...
| `delegator` | Defers to MindForge's recommendation. Trusts the suggestion. |

**Signal patterns:**

1. **Response latency to options** -- How many messages between MindForge presenting options and developer choosing. Immediate (same message or next) suggests fast-intuitive.
...
4. If developer uses delegation language (> 3 instances) AND rarely overrides MindForge's choices AND says "sounds good" or "your call" --> `delegator`
...
**What we're measuring:** How the developer approaches problems, errors, and unexpected behavior when working with MindForge.
...
| `hypothesis-driven` | Investigates independently first, brings specific theories to MindForge for validation. |
| `collaborative` | Wants to work through the problem step-by-step with MindForge as a partner. |
...
3. If developer shares their own analysis AND proposes theories ("I think the issue is X because...") AND asks MindForge to confirm or refute --> `hypothesis-driven`
...
| Rating | Description |
|--------|-------------|
| `pragmatic-fast` | Uses what works, what MindForge suggests, or what's fastest. Minimal evaluation. |
...
4. **Rejection patterns** -- Does the developer reject MindForge's suggestions? On what basis (popularity, personal experience, docs quality)?
...
4. If developer names specific libraries unprompted AND overrides MindForge's suggestions AND expresses strong preferences --> `opinionated`
...
**What we're measuring:** What causes visible frustration, correction, or negative emotional signals in the developer's messages to MindForge.

**Rating spectrum:**

| Rating | Description |
|--------|-------------|
| `scope-creep` | Frustrated when MindForge does things that were not asked for. Wants bounded execution. |
| `instruction-adherence` | Frustrated when MindForge doesn't follow instructions precisely. Values exactness. |
| `verbosity` | Frustrated when MindForge over-explains or is too wordy. Wants conciseness. |
| `regression` | Frustrated when MindForge breaks working code while fixing something else. Values stability. |
...
1. If developer corrects MindForge for doing unrequested work AND uses language like "I only asked for X", "stop adding things", "stick to what I asked" --> `scope-creep`
...
3. If developer asks MindForge to be shorter AND skips explanations AND expresses annoyance at length ("too much", "just the answer") --> `verbosity`
...
| Rating | Description |
|--------|-------------|
| `self-directed` | Reads code directly, figures things out independently. Asks MindForge specific questions. |
| `guided` | Asks MindForge to explain relevant parts. Prefers guided understanding. |
...
2. If developer asks MindForge to explain concepts AND requests walkthroughs AND prefers Claude-mediated understanding --> `guided`
...
```json
{
  ...
  "dimensions": {
    "communication_style": {
      ...
      "claude_instruction": "Imperative directive for MindForge: 'Match structured communication style' not 'You tend to provide structured context'"
    },
    "decision_speed": {
...
- **`claude_instruction`**: Must be written in imperative form directed at MindForge. This field is how the profile becomes actionable.
...
- "Context-dependent: terse-direct for CLI/backend projects (mindforge-tools, api-server), detailed-structured for frontend projects (dashboard, landing-page)."
...
Context-dependent splits are resolved during Phase 3 orchestration. The orchestrator presents the split to the developer and asks which pattern represents their general preference. Until resolved, MindForge uses the dominant pattern with awareness of the context-dependent variation.
