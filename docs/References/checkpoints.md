...
**Core principle:** MindForge automates everything with CLI/API. Checkpoints are for verification and decisions, not manual work.

**Golden rules:**
1. **If MindForge can run it, MindForge runs it** - Never ask user to execute CLI commands, start servers, or run builds
2. **MindForge sets up the verification environment** - Start dev servers, seed databases, configure env vars
3. **User only does what requires human judgment** - Visual checks, UX evaluation, "does this feel right?"
4. **Secrets come from user, automation comes from MindForge** - Ask for API keys, then MindForge uses them via CLI
...
**When:** MindForge completed automated work, human confirms it works correctly.
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[What MindForge automated and deployed/built]</what-built>
  <how-to-verify>
...
**Example: UI Component (shows key pattern: MindForge starts server BEFORE checkpoint)**
```xml
<task type="auto">
...
**When:** Action has NO CLI/API and requires human-only interaction, OR MindForge hit an authentication gate during automation.

**Use ONLY for:**
- **Authentication gates** - MindForge tried CLI/API but needs credentials (this is NOT a failure)
...
<task type="checkpoint:human-action" gate="blocking">
  <action>[What human must do - MindForge already did everything automatable]</action>
  <instructions>
    [What MindForge already automated]
    [The ONE thing requiring human action]
  </instructions>
  <verification>[What MindForge can check afterward]</verification>
...
<!-- If vercel returns "Error: Not authenticated", MindForge creates checkpoint on the fly -->
...
<!-- After authentication, MindForge retries the deployment -->
...
**Key distinction:** Auth gates are created dynamically when MindForge encounters auth errors. NOT pre-planned — MindForge automates first, asks for credentials only when blocked.
</type>
</checkpoint_types>

<execution_protocol>

When MindForge encounters `type="checkpoint:*"`:
...
**Auth gate = MindForge tried CLI/API, got auth error.** Not a failure — a gate requiring human input to unblock.

**Pattern:** MindForge tries automation → auth error → creates checkpoint:human-action → user authenticates → MindForge retries → continues
...
**Key distinction:**
- Pre-planned checkpoint: "I need you to do X" (wrong - MindForge should automate)
...
**The rule:** If it has CLI/API, MindForge does it. Never ask human to perform automatable work.
...
<!-- RIGHT: MindForge asks for value, then adds via CLI -->
<task type="checkpoint:human-action">
...
| Action | Automatable? | MindForge does it? |
|--------|--------------|-----------------|
...
**DON'T:**
- Ask human to do work MindForge can automate ❌
- Assume knowledge: "Configure the usual settings" ❌
- Skip steps: "Set up database" (too vague) ❌
- Mix multiple verifications in one checkpoint ❌

**Placement:**
- **After automation completes** - not before MindForge does the work
...
<!-- NO CHECKPOINT NEEDED - MindForge automated everything and verified programmatically -->
```
...
**Why bad:** MindForge can run `npm run dev`. User should only visit URLs, not execute commands.

### ✅ GOOD: MindForge starts server, user visits
...
### ❌ BAD: Asking human to deploy / ✅ GOOD: MindForge automates
...
<!-- GOOD: MindForge deploys, user verifies -->
<task type="auto">
  <name>Deploy to Vercel</name>
...
**Why bad:** MindForge can run these commands. User should never execute CLI commands.
...
**Why bad:** Stripe has an API. MindForge should create the webhook via API and write to .env directly.
...
**The golden rule:** If MindForge CAN automate it, MindForge MUST automate it.

**Checkpoint priority:**
1. **checkpoint:human-verify** (90%) - MindForge automated everything, human confirms visual/functional correctness
2. **checkpoint:decision** (9%) - Human makes architectural/technology choices
3. **checkpoint:human-action** (1%) - Truly unavoidable manual steps with no API/CLI

**When NOT to use checkpoints:**
- Things MindForge can verify programmatically (tests, builds)
- File operations (MindForge can read files)
