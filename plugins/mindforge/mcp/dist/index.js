...
const memory_js_1 = require("./vendor/memory.js");
const client_js_1 = require("./vendor/client.js");
...
const memory = () => new memory_js_1.MindForgeMemory(PROJECT_ROOT);
const client = () => new client_js_1.MindForgeClient({ projectRoot: PROJECT_ROOT });
...
const server = new mcp_js_1.McpServer({ name: 'mindforge', version: '11.3.1' });
...
registerTool('mindforge_health', {
    title: 'MindForge project health',
    description: 'Run a MindForge health check on the current project: verifies required ' +
...
registerTool('mindforge_status', {
    title: 'MindForge project status',
    description: 'Read the current MindForge project status: whether the project is initialized, ' +
...
const memoryQuerySchema = {
    topic: zod_1.z.string().optional().describe('Free-text topic to match against entries'),
    tags: zod_1.z.array(zod_1.z.string()).optional().describe('Tags to match (boosts ranking)'),
    type: zod_1.z.enum(KNOWLEDGE_TYPES).optional().describe('Restrict to one knowledge type'),
    minConfidence: zod_1.z.number().min(0).max(1).optional().describe('Minimum confidence (0-1, default 0.3)'),
    limit: zod_1.z.number().int().positive().max(100).optional().describe('Max results (default 20)'),
    includeGlobal: zod_1.z.boolean().optional().describe('Include the cross-project global knowledge base'),
};
registerTool('mindforge_memory_query', {
    title: 'Query MindForge knowledge base',
    description: 'Search the MindForge knowledge graph (architectural decisions, code/bug patterns, ' +
...
registerTool('mindforge_memory_stats', {
    title: 'MindForge memory statistics',
    description: 'Report statistics for the MindForge knowledge graph: total/active/deprecated ' +
...
registerTool('mindforge_memory_find_related', {
    title: 'Find related MindForge knowledge',
    description: 'Given a free-text query, find related knowledge entries via keyword scoring plus ' +
    ...
    inputSchema: {
        query: zod_1.z.string().min(1).describe('The task or topic to find related knowledge for'),
        maxHops: zod_1.z.number().int().min(0).max(5).optional().describe('Graph traversal depth (default 2)'),
        topK: zod_1.z.number().int().positive().max(50).optional().describe('Max results (default 10)'),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
...
const auditLogSchema = {
    event: zod_1.z.string().optional().describe('Filter by event type (e.g. task_completed, security_finding)'),
    phase: zod_1.z.number().int().optional().describe('Filter by phase number'),
    limit: zod_1.z.number().int().positive().max(500).optional().describe('Max entries to return (default 50, newest last)'),
};
registerTool('mindforge_audit_log', {
    title: 'Read MindForge audit log',
    description: 'Read entries from the MindForge audit log (.planning/AUDIT.jsonl), optionally ' +
...
registerTool('mindforge_memory_remember', {
    title: 'Store a MindForge knowledge entry',
    description: 'Persist a new knowledge entry (decision, pattern, preference, or domain note) into ' +
    ...
    inputSchema: {
        type: zod_1.z.enum(KNOWLEDGE_TYPES).describe('The kind of knowledge being stored'),
        topic: zod_1.z.string().min(1).describe('Short topic/title (truncated to 80 chars)'),
        content: zod_1.z.string().min(1).describe('The knowledge content to remember'),
        confidence: zod_1.z.number().min(0).max(1).optional().describe('Confidence 0-1 (default 0.7)'),
        tags: zod_1.z.array(zod_1.z.string()).optional().describe('Tags for later retrieval'),
    },
